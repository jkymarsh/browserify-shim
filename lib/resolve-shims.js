...
// inspect the currently-running browserify process for the location of its running 'cmd.js'
function discoverBrowserifyProcess() {
  for (var i = 0; i < process.argv.length; i++) {
    if (process.argv[i].indexOf('browserify') > -1) {
      return process.argv[i];
    }
  }
}

// check to see if the current module being inspected for shimming resides within the same parent
//  directory as the currently-running browserify process
function validateBuiltinModule(file) {
  var browserifyDirPath = fs.realpathSync(path.resolve(discoverBrowserifyProcess(), '..', '..'));
  var resolvedModulePath = fs.realpathSync(path.resolve(file, '..'));

  return (resolvedModulePath.indexOf(browserifyDirPath) > -1);
}
...
var resolve = module.exports = function resolveShims (file, messages, cb) {
  ...
  mothership(file, function (pack) { return !! pack['browserify-shim'] }, function (err, res) {
    ...
    // if module and its respective package.json shim config exist, proceed to shimming
    if (res && res.pack) {
      var pack       = res.pack;
    ...
    // else, if the module has a built-in node equivalent, don't shim at all
    else if (validateBuiltinModule(file)) {
      console.warn('has built-in node module; skipping shim step');

      // pass an empty object to the callback so the 'exposeGlobals' and 'shim' properties default to falsy,
      //  resulting in the unaltered module being written to the stream without shimming
      cb(null, {});
    }
    // else, if it has no config AND doesn't have a built-in equivalent, throw an error
    else {
      return cb(new Error('Unable to find a browserify-shim config section in the package.json for ' + file));
    }
